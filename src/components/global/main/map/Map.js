...
class MapCore extends React.Component {
  ...
  render() {
    ...
    this.props.roominfo.dynamic.forEach((region, index) => {
      ...
      for (var j = 0; j < this.props.roominfo.static[index].data.mapTextItems.length; j++) {
        let obj = this.props.roominfo.static[index].data.mapTextItems[j];
        let position = this.convert(this.props.roominfo.static[index].regionId, obj.x, obj.y);
