...
var steamidlist = [];
var socketidlist = [];
//logger.info(steamidlist);
//logger.info(socketidlist);
    ...
    if (db.logincheck(socket.handshake)) {
      ...
      if (link === conf.fhghq.url) {
        ...
        if (rank > 0 && rank < 5) {
          ...
          socket.join(link);
          //logger.info("User "+account.id+" pushed");
          steamidlist.push(account.id);
          socketidlist.push(socket.id);
          //logger.info(steamidlist);
          //logger.info("Socket "+socket.id+" joined room "+link);
...
exports.connectDiscord = function (globalid, link) {
  io.in(globalid)
    .emit('changeSettings', 'link', link);
  io.in(globalid)
    .emit('deleteSettings', 'token');
};
...
function UpdateStats() {
  ...
  request.open('GET', `${conf.warApi.liveUrl}/api/worldconquest/war`, true);
...
exports.FinishUpdateStats = function () {
  let packet = {
    totalplayers: exports.totalplayers,
    warstats: exports.warstats,
    wr: warapi.WR,
    currentwar: exports.currentwar
  };
  io.to('stats')
...
//Updates users' status in their profiles
  for (var i = 0; i < users.length; i++) {
    ...
    io.in(users[i].id)
      .emit('updateroom', packet);
    //logger.info(users[i].id);
    //logger.info(socketidlist[0]);
    //logger.info(io.sockets.connected);
...
function getglobalid(socket) {
  ...
  var globalid = socket.handshake.headers.referer.replace(prefix + 'room/', '');
  return globalid;
}
